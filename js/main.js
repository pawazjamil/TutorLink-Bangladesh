        ...
        document.getElementById('profileContent').innerHTML = `
            ...
            <div style="margin-top:2rem;">
                ...
                ${userBookings.length === 0 ? '<p>No bookings yet.</p>' : 
                    userBookings.map(b => `<div style="background:#f5f5f5; padding:1rem; margin:0.5rem 0; border-radius:10px;">
...
function applyAsTutor() {
    ...
    const newTutor = { id: tutors.length + 1, name, email, password, type: 'tutor', phone, subject, baseRate: rate, perDayRate: Math.round(rate / 7), location, qualification, experience: 1, bio, avatar: '👨‍🏫', rating: 0, totalRatings: 0, reviews: [], profilePic: null };
...
function loadConversations() {
    ...
    const sortedConversations = Array.from(conversations.values()).sort((a, b) => new Date(b.lastTime) - new Date(a.lastTime));
        ...
        <div style="display: flex; gap: 2rem; flex-wrap: wrap;">
            <div style="flex: 1; min-width: 280px;">
                ...
                <div id="conversationsList">
                    ${sortedConversations.length === 0 ? '<p>No conversations yet.</p>' : 
...
function populateUserList(searchTerm = '') {
    ...
    if (searchTerm) {
        usersList = usersList.filter(u => u.name.toLowerCase().includes(searchTerm.toLowerCase()));
    }
...
function searchTutors() {
    ...
    if (subjectInput !== "") {
        filtered = filtered.filter(t => t.subject.toLowerCase() === subjectInput.toLowerCase());
    }
    
    if (locationInput !== "") {
        filtered = filtered.filter(t => t.location.toLowerCase().includes(locationInput.toLowerCase()));
    }
...
function confirmBooking() {
    ...
    const days = parseInt(document.getElementById('bookingDays').value);
    const total = currentBookingTutor.baseRate + (days - 4) * currentBookingTutor.perDayRate;
    bookings.push({ id: Date.now(), tutor: currentBookingTutor.name, tutorEmail: currentBookingTutor.email, subject: currentBookingTutor.subject, days, total, date: new Date().toLocaleDateString(), studentName: currentUser.name, studentEmail: currentUser.email });
    localStorage.setItem('bookings', JSON.stringify(bookings));
    showToast(`Booked ${currentBookingTutor.name} for ৳${total}/month`);
    closeModal('bookingModal');
    
    messages.push({ id: Date.now(), from: currentUser.email, fromName: currentUser.name, to: currentBookingTutor.email, toName: currentBookingTutor.name, content: `I have booked your ${days} days/week package for ৳${total}/month.`, timestamp: new Date().toISOString(), read: false });
    localStorage.setItem('messages', JSON.stringify(messages));
}
...
function setRating(r) {
    selectedRating = r;
    document.querySelectorAll('.stars-input span').forEach((s, i) => { if (i < r) s.classList.add('active'); else s.classList.remove('active'); });
}
